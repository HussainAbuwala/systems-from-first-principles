...
export async function POST(request: Request) {
  ...
  await env.DB.prepare(
    "INSERT INTO experiments (id, version, initial_stock, available, created_at) VALUES (?, ?, 1, 1, ?)",
  ).bind(id, version, Date.now()).run();

  return NextResponse.json({ id, version, initialStock: 1, available: 1 });
}
